            ...
            <div class="formControls col-xs-8 col-sm-9">
				...
				<select class="select" size="1" name="country_id">
                    ...
                    @foreach($country as $val)
                        <option value="{{$val-> id}}" selected>{{$val-> area}}</option>
                    @endforeach
				</select>
				</span>
                <span class="select-box" style="width: 110px;">
...
<script type="text/javascript" src="/static/avatar.js"></script>
<!--/_footer 作为公共模版分离出去-->
...
<script type="text/javascript">
    $(function(){
        ...
        $('select[name=country_id]').change(function (){
            var id = $(this).val();
            $.get('/admin/member/getarebyid',{id: id},function (data) {
                var str = '';
                $.echo(data,function (index,el) {
                    str += "<option value='"+el.id+"' selected>"+ el.area +"</option>"
                });
